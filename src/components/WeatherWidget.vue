...
const handleCitiesUpdate = async (newCities: CityConfig[]) => {
  const oldCities = cities.value;
  const addedCity = newCities.find(nc => !oldCities.some(oc => oc.name === nc.name));

  if (addedCity) {
    try {
      const coords = await weatherApi.getCoordsByCityName(addedCity.name);
      if (coords) {
        const newCityWithCoords: CityConfig = { ...addedCity, lat: coords.lat, lon: coords.lon };
        const finalCities = newCities.map(city => city.name === addedCity.name ? newCityWithCoords : city);

        cities.value = finalCities.map((city, index) => ({ ...city, order: index }));
        ElMessage.success('City added successfully');
        saveConfig();
      }
    } catch (error) {
      ElMessage.error('City not found or API error');
      cities.value = oldCities;
    }
  } else {
    ...
    if (!cities.value.some(city => city.name === currentCity.value)) {
      currentCity.value = cities.value[0]?.name || '';
      if (currentCity.value && !showSettings.value) {
        loadWeatherData();
      }
    }
  }
